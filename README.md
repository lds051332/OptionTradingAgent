...
美股个股、短周期卖 Put 的**每周决策台**。给建议，不报单。

CLI 和浏览器共用同一条流水线：拉链 → 日历门控 → 事件侦察 → 结构化终审。默认策略面向 `NVDA` / `MSFT`，DTE 3–9 天，结构是 Cash-secured Put 或 Bull Put Spread。

> **不是投资建议，也不是自动交易。** 输出只给人工复核。期权可能亏掉全部权利金，CSP 被指派时还要准备现金。
...
- 经纪商下单、持仓同步、盘中监控
- 多用户账号 / OAuth、历史会话落库
- aggressive（>0.25Δ）档、Call、指数/加密期权
